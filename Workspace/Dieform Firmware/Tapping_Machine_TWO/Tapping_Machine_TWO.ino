...
const int p_clamp =  A4;  
const int p_unclamp =  A3;
...
void loop() {
  ...
  if (part<40 && PsideOut>100 && PupOut>100 && PheadIn<100 && state==0){                    //check if part is loaded 
    delay(500);
    readSensors();
    if (part<100){
      state = 1;
  ...
  if(Punclamp<100 && PheadIn<100 && PupOut>100  && state==1 ){              //birng part to clamp
    digitalWrite(side,LOW);
    state = 2;
    Serial.println(state);
  }

  if (PupOut>100 && PsideIn>100 && Punclamp<100 && state==2){             //clamp condition 
    delay(300);
...
//make return2home func
  if(Punclamp>100 && PheadIn<100 && state==3){                                          //return to home Pclamp==1 &&
    delay(300);
  ...
  if(PheadOut<100 && Punclamp>100 && state ==6){
    digitalWrite(switch1,HIGH);
    state = 7;
    Serial.println(state);
  }

  if(PheadIn<100 && state==7){                   //Unclamp condition
